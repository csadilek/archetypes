...
public class CDITestHelper {
  ...
  public void countMessageEvent(@Observes HelloMessage message) {
    ++messageEventCount;
  }
  
  public void countResponseEvent(@Observes Response response) {
    ++responseEventCount;
  }
  
  public static void afterResponseEvent(final Runnable runnable) {
    final int origResponseEventCount = responseEventCount;
    final Timer t = new Timer() {
      @Override
      public void run() {
        if (responseEventCount > origResponseEventCount) {
          runnable.run();
        } else {
          // poll again later
          schedule(500);
